...
async function main() {
  ...
  while (true) {
    const { data: gs, error: gsErr } = await admin
      .from("game_state")
      .select(
        "phase, current_question_id, current_round_id, is_paused",
      )
    ...
    if (phaseKey !== prevKey) {
      ...
      console.log(
        `[+${elapsed}s] phase=${gs.phase} round=${gs.current_round_id?.slice(0, 8) ?? "-"} q=${gs.current_question_id?.slice(0, 8) ?? "-"} paused=${gs.is_paused}`,
      );
      lastPhase = gs.phase;
      ...
      if (!phaseScreenshots.has(phaseKey)) {
        ...
        const tag = `${gs.phase}-q${gs.current_question_id?.slice(0, 8) ?? "none"}`;
        await Promise.all(
          pages.map((page, i) =>
            page
