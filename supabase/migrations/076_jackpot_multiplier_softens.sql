-- ============================================================
-- Migration 076: Jackpot modifier — soften for large events
-- ============================================================
--
-- WHAT THIS CHANGES:
--   Jackpot modifier previously awarded:
--     • First correct answer → base_points × multiplier (default 5×)
--     • All other correct answers → 0 points
--     • Wrong answers → 0 points
--
--   In a 200-player event this is brutal. 199 people who got it right
--   feel like the game punished them. The "race to be first" drama is
--   fun, the "you got it right but still got zero" outcome is not.
--
--   New behaviour:
--     • First correct answer → base_points × multiplier (unchanged, default 5×)
--     • All other correct answers → STANDARD scoring (base + speed bonus,
--       exactly as if no modifier were active)
--     • Wrong answers → 0 points (unchanged)
--
--   The Jackpot winner still feels like they crushed it (5× their normal
--   score) without the rest of the room going home empty-handed.
--
-- WHY THIS SITS IN submit_answer:
--   The Jackpot branch runs inline in the RPC (see migration 075). We patch
--   only that branch — the rest of the function (single-read JOIN, round-type
--   dispatch, idempotency) is untouched.
--
-- ROLLBACK:
--   Re-run migration 075 to restore the all-or-nothing Jackpot behaviour.
-- ============================================================
...
BEGIN
  ...
  -- ── Jackpot modifier (REBALANCED — migration 076) ──────────────────────────
  -- First correct → base × multiplier (e.g. 5× with speed bonus applied).
  -- Other correct → STANDARD base + speed bonus (not zero).
  -- Wrong → 0. This keeps the "race to be first" drama without punishing
  -- the 199/200 players who got it right but weren't the fastest.
  IF v_modifier_type = 'jackpot' THEN
    ...
    IF v_is_correct THEN
      -- Compute the standard points first (base + speed bonus if enabled).
      -- This is the baseline the multiplier amplifies for the winner.
      ...
      -- First-correct check: is there any OTHER correct answer on this question already?
      IF NOT EXISTS (
        ...
        -- Winner: amplify the full (base + speed bonus) score by the multiplier.
        v_jackpot_winner := true;
        v_points := FLOOR(v_points * v_jackpot_multiplier);
      END IF;
      -- else: v_points already holds the standard base + speed score — leave as-is.
    ELSE
      -- Wrong answer — still zero points.
      v_points := 0;
    END IF;
