-- ============================================================
-- Migration 077: submit_answer returns modifier_applied
-- ============================================================
--
-- WHY:
--   The reveal-phase UI was reading gs.modifier_state to decide whether
--   to show jackpot copy. Problem: the host can activate jackpot AFTER
--   players have already submitted on the current question (e.g.
--   between submit and reveal). modifier_state then bleeds into the
--   reveal of a question where no jackpot was active during scoring,
--   producing the false banner "Jackpot claimed — scores settled
--   normally" on a question that had nothing to do with jackpot.
--
--   Reproduced on prod game WY5WT (R2.Q1, 2026-04-19): players answered
--   at 16:53:45, jackpot activated at 16:53:50, reveal showed jackpot
--   banner even though scoring used the standard branch.
--
-- WHAT THIS CHANGES:
--   submit_answer now returns `modifier_applied` (text or null) — the
--   modifier_type that was actually resolved at scoring time. The
--   client uses this (not live game_state) to decide reveal-phase copy.
--
-- ROLLBACK:
--   Re-run 076. The extra return field is additive; older clients
--   simply ignore it.
-- ============================================================
...
BEGIN
  ...
  IF v_round_type = 'closest_wins' THEN
    ...
    RETURN jsonb_build_object(
      ...
      'modifier_applied', v_modifier_type
    );
  END IF;
